...
class BoxPlot:
    ...
    def plot(
        ...
        visit_colors = {
            'Healthy Controls': '#00470d',
            'Pre-Treatment (PreTx)': '#8b0000',
            'Post-Induction Round 2 (PI2C)': '#810272',
            'End of Induction (EI)': '#7502c2',
            '60 days Post-Transplant (ASCT60d)': '#0020b0',
            '90 days Post-Transplant (ASCT90d)': '#2345de',
            '1 year Post-Transplant (ASCT1y)': '#0389ff',
            '2 years Post-Transplant (ASCT2y)': '#008080'
        }

        df_conditions = df[condition].unique()
        condition_groups = []
        for visit in visit_colors.keys():
            if visit in df_conditions:
                condition_groups.append(visit)
        ...
        if not perturbation:
            ...
            for condition_group in condition_groups:
                fig.add_trace(
                    go.Box(
                        y=df.filter(pl.col(condition) == condition_group)["Score"].to_list(),
                        x=[condition_group] * df.filter(pl.col(condition) == condition_group).height,
                        name=condition_group,
                        boxpoints="all",
                        pointpos=0,
                        fillcolor='rgba(0,0,0,0)',
                        line_color=visit_colors[condition_group],
                        line_width=2,
                        showlegend=False
                    )
                )
        ...
        fig.update_layout(
            ...
            title=title,
            xaxis_title="Visit",
            yaxis_title="GSVA Score",
            margin=dict(t=55,l=70,r=75,b=75),
